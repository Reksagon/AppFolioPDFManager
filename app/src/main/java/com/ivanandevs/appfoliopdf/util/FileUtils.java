...
public class FileUtils {
    ...
    public void shareFile(File file) {
        Uri uri = FileProvider.getUriForFile(mContext, Constants.AUTHORITY_APP, file);
    ...
    public boolean isFileExist(String mFileName) {
        String path = mSharedPreferences.getString(Constants.STORAGE_LOCATION,
    ...
    public static String getFileName(String path) {
        ...
        int index = path.lastIndexOf(Constants.PATH_SEPERATOR);
    ...
    public static String getFileNameWithoutExtension(String path) {
        if (path == null || path.lastIndexOf(Constants.PATH_SEPERATOR) == -1)
            return path;

        String filename = path.substring(path.lastIndexOf(Constants.PATH_SEPERATOR) + 1);
        filename = filename.replace(Constants.pdfExtension, "");
    ...
    public static String saveImage(String filename, Bitmap finalBitmap) {
        ...
        File myDir = new File(root + Constants.pdfDirectory);
